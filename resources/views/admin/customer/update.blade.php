                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="customer_name" id="customer-name" value="{{ $customer->name }}">
                            ...
                            <div class="form-group row">
                                <label for="contract-code" class="col-sm-3 col-form-label">Mã hợp đồng<span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="contract_code" id="contract-code" value="{{ $customer->contract_code }}">
                                    ...
                                    @if(count($product))
                                        @foreach($product as $key => $value)
                                            <input type="checkbox" name="product_name[]" id="product-name-{{ $value->id }}"
                                                   value="{{ $value->id }}"
                                                <?php
                                                    $product_id = json_decode($customer->product_id, true);
                                                    if(in_array($value->id, $product_id)) echo 'checked';
                                                    else echo '';
                                                    ?> />
                                            <label class="mr-4" for="product-name-{{ $value->id }}">{{ $value->name }}</label>
                                        @endforeach
                                    @endif
                                    <br>
                                    ...
                                    <select class="form-control" name="photographer" id="photographer">
                                        ...
                                        @if(count($photographer))
                                            @foreach($photographer as $key => $value)
                                                <option value="{{ $value->id }}" {{ $customer->photographer_id == $value->id ?'selected':'' }}>{{ $value->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    ...
                                    <select class="form-control" name="makeup" id="makeup">
                                        ...
                                        @if(count($makeup))
                                            @foreach($makeup as $key => $value)
                                                <option value="{{ $value->id }}" {{ $customer->makeup_id == $value->id ?'selected':'' }}>{{ $value->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    ...
                                    <select class="form-control" name="shopper" id="shopper">
                                        ...
                                        @if(count($shopper))
                                            @foreach($shopper as $key => $value)
                                                <option value="{{ $value->id }}" {{ $customer->shopper_id == $value->id ?'selected':'' }}>{{ $value->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="form-group row">
                                <label for="photography-date" class="col-sm-3 col-form-label">Ngày chụp <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control date" readonly name="photography_date" id="photography-date" value="{{ date('d-m-Y', strtotime($customer->photography_date)) }}" placeholder="dd/mm/yyyy">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <textarea class="form-control" name="note" rows="3" id="note">{{ $customer->note }}</textarea>
                                    @if($errors->has('note'))
                                        <span class="text-danger error-text">
                                        {{$errors->first('note')}}
                                    </span>
