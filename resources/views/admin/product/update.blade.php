                        ...
                        <div class="col-md-12">
                                    ...
                                    <select class="form-control" name="branch" id="branch">
                                        ...
                                        @if(count($branch))
                                            @foreach($branch as $key => $value)
                                                <option value="{{ $value->id }}" {{ $product->branch_id==$value->id ? 'selected':'' }}>{{ $value->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            <div class="form-group row">
                                <label for="price" class="col-sm-2 col-form-label">Giá <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-10">
                                    <input type="number" min="1" class="form-control" name="price" id="price" value="{{ number_format($product->price, 0, '', '.') }}" >
                                    @if($errors->has('price'))
                                        <span class="text-danger error-text">
                                            {{$errors->first('price')}}
                                        </span>
                            ...
                            <div class="form-group row">
                                <label for="quantity" class="col-sm-2 col-form-label">Số lượng <span
                                        class="text-danger">*</span></label>
                                <div class="col-sm-10">
                                    <input type="number" min="1" class="form-control" name="quantity" id="quantity" value="{{ $product->quantity }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10 form-tags">
                                    <select class="form-control" name="active" id="active">
                                        <option value="true" {{ $product->active == 1 ? 'selected':'' }}>Có</option>
                                        <option value="false" {{ $product->active == 0 ? 'selected':'' }}>Không</option>
                                    </select>
                                </div>
                            </div>
