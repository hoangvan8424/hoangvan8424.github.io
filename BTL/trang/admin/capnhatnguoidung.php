<?php
	...
	if(isset($_POST['cnnd'])) {
	    ...
	    if(mysql_num_rows($query) == 0){	
			$sql = "UPDATE `htnshop`.`nguoidung` SET `hoten` = '$hoten', `password` = '$mk', `ngaysinh` = '$ngaysinh',`gioitinh` = '$gt', `email`='$email', `phanquyen` = '$quyen' WHERE `nguoidung`.`maND` = $id";
   
	        $count =  mysql_query($sql);
	        if($count) {
	        	echo "<script>
					alert('Cập người dùng thành công');
...
<h5 class="h-themnd text-center mt-2">Cập nhật người dùng</h5>
<form class="them-nd" name="form-tnd" method="POST" action="">
	<label>Email</label><input type="email" name="email" class="ip" value="<?php echo $row[5]?>">
	<label>Mật khẩu</label><input type="password" name="matkhau" class="ip mt-sm-4" value="<?php echo $row[2]?>">
	<label>Nhập lại mật khẩu</label><input type="password" name="re-matkhau" class="ip mt-sm-4" value="<?php echo $row[2]?>">
	<label>Họ tên</label><input type="text" name="hoten" class="ip mt-sm-4" value="<?php echo $row[1]?>">
	...
	<!-- <input type="radio" name="gt" class="gt ml-5" value="Nữ"><span>Nữ</span><br> -->
	<label>Quyền</label>
	...
	<div class="btn-tnd mb-3">
		<button class="btn btn-success btn-sm mt-sm-5 mr-3" type="submit" name="cnnd">Cập nhật</button>
