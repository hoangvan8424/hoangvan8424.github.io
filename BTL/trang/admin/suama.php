<?php
	if(isset($_GET['id'])) {
		$maSP = $_GET['id'];
		$sql = "SELECT *FROM capnhatma WHERE maSP=".$maSP;
	...
	if(isset($_POST['btncnsp'])) {
		...
		if($kq) {		
			...
			$bcb = $_POST['bcb'];
			$mhma = $_POST['mhma'];
			$sqlis = "UPDATE chitietma SET dophangia='$dpg', bocambien='$bcb', manhinh='$mhma' WHERE machitietMA='$mact' AND maSP='$id' ";
			mysql_query($sqlis);
...
<form class="ml-md-3 mr-md-3 them-sp" name="form-cnsp" method="POST" action="" enctype="multipart/form-data">
	...
	<label>Hình ảnh</label><input type="file" name="hinhanh" id="file" class="ip mt-sm-4"><br>
	...
	<select name="chontrangthai" class="mt-sm-4">
		<?php
			if($row[6]==0) {
				echo '	<option value="chontrangthai">Chọn</option>
	...
	<div>
		<label class="dpg">Độ phân giải</label><textarea type="text" name="dpg" class="ip mt-sm-4"><?php echo $row[11] ?></textarea>
		<div id="n-dpg" class="text-center"></div>
		<label class="bcb">Bộ cảm biến</label><textarea type="text" name="bcb" class="ip mt-sm-4"><?php echo $row[12] ?></textarea>
		<div id="n-bcb" class="text-center"></div>
		<label class="mhma">Màn hinh</label><textarea type="text" name="mhma" class="ip mt-sm-4"><?php echo $row[13] ?></textarea>
