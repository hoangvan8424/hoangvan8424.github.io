<?php
	if(isset($_POST['btnthem'])) {
		...
		$sql = "INSERT INTO dacdiemmt(maSP, thietke, manhinh, cauhinh, baomat, anh_1, anh_2, anh_3, anh_4)
...
?>
<h5 class="h-cnsp text-center mt-2">Thêm đặc điểm nổi bật máy tính</h5>
<form class="ml-md-3 mr-md-3 them-sp" name="themsp" onsubmit="return checkFormThemSP()" method="POST" action="" enctype="multipart/form-data">
	...
	<div>
		...
		<label class="cpu">Màn hình</label><textarea type="text" name="manhinh" class="ip mt-sm-4"></textarea>
		<div id="n-mh" class="text-center"></div>
		<label class="ram">Cấu hình</label><textarea type="text" name="cauhinh" class="ip mt-sm-4"></textarea>
		<div id="n-ch" class="text-center"></div>
		<label class="ram">Bảo mật</label><textarea type="text" name="baomat" class="ip mt-sm-4"></textarea>
...
<script type="text/javascript">
	...
	function checkFormThemSP() {
		...
		if(ch=='') {
			document.getElementById('n-ch').innerHTML='<span class="text-danger">Cấu hình không được để trống!</span>';
			form['cauhinh'].focus();
			return false;
		}
		else {
		...
		if(bm=='') {
			document.getElementById('n-bm').innerHTML='<span class="text-danger">Bảo mật không được để trống!</span>';
			form['baomat'].focus();
			return false;
		}
		else {
