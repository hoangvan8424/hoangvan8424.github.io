...
<?php
	if(isset($_GET['id'])){
		$id = $_GET['id'];
		$sql="SELECT *FROM ddmaytinh WHERE maDDNBMT=".$id;
	...
	if(isset($_POST['btncn'])) {
		...
		$sql2 = "UPDATE dacdiemmt SET thietke='$thietke', manhinh='$manhinh', cauhinh='$cauhinh', baomat='$baomat',
		 			anh_1='$anh1', anh_2='$anh2', anh_3='$anh3', anh_4='$anh4' WHERE maDDNBMT=".$id;
		$kq=mysql_query($sql2);
		if($kq) {
			echo '	<script>
...
?>
<h5 class="h-cnsp text-center mt-2">Cập nhật đặc điểm nổi bật máy tính</h5>
<form class="ml-md-3 mr-md-3 them-sp" name="themsp" onsubmit="return checkFormThemSP()" method="POST" action="" enctype="multipart/form-data">
	...
	<div>
		...
		<label class="cpu">Màn hình</label><textarea type="text" name="manhinh" class="ip mt-sm-4"><?php echo $row[8]; ?></textarea>	
		<label class="ram">Cấu hình</label><textarea type="text" name="cauhinh" class="ip mt-sm-4"><?php echo $row[9]; ?></textarea>	
		<label class="ram">Bảo mật</label><textarea type="text" name="baomat" class="ip mt-sm-4"><?php echo $row[10]; ?></textarea>
