...
<div id="ql-sanpham" class="mt-3"> <!-- bắt đầu quản lý sản phẩm -->
	<div>
		...
		<p>Tổng sản phẩm:
			<?php
				$sql="SELECT maSP, maDM FROM sanpham";
	...
	<table class="table table-hover table-inverse table-responsive-md text-center" style="font-size: 14px;">
		...
		<tbody>
			<?php
				...
				$sql="	SELECT * 
						FROM sanpham AS sp, chitietmt AS ctmt
						WHERE sp.maSP = ctmt.maSP LIMIT $from, $sosp1trang";
				$query= mysql_query($sql);
			...
			<tr>
				...
				<td><a href="admin.php?module=suamt&id=<?php echo $row['maSP'];?>" type="button" class="btn btn-primary btn-sm"><i class="fas fa-sync-alt"></i></a></td>

				<td><a type="button" onclick="return confirm('Xóa?')" href="admin.php?module=xoamt&id=<?php echo $row['maSP'];?>&mact=<?php echo $row['machitietSP'] ?>" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i></a></td>
				<td><a href="admin.php?module=themmt" type="button" class="btn btn-success btn-sm"><i class="fas fa-plus"></i></a></td>
			</tr>
	...
	<?php
		$x = mysql_query("SELECT maSP FROM chitietmt");
