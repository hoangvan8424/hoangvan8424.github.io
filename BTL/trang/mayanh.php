...
<head>
	...
	<meta name="viewport" content="width=device-width, initial-scale=1">
  	...
  	<link rel="stylesheet" type="text/css" href="../style/mayanh.css">
		...
		<div class="row"> <!-- bắt đầu row -->
			...
			<div class="col-sm-3 mt-3"> <!-- bắt đầu cột đầu tiên - 3 -->
				...
				<div class="bo-loc" style="background: #fff;"> <!-- bắt đầu bộ lọc -->
					...
					<div class="row mt-3"> <!-- bắt đầu lọc theo hãng sản xuất -->
						<div class="col-12">
							<h6 class="ml-2">Hãng sản xuất</h6>
							<?php 
							
								$sql ="SELECT DISTINCT tenNCC from ncc where maNCC=3 or maNCC=4";
					...
					</div> <!-- kết thúc lọc theo hãng sản xuất -->
					...
					<div class="row mt-3"> <!-- bắt đầu lọc theo độ phân giải -->
						<div class="col-12">
							<h6 class="ml-2">Độ phân giải</h6>
							<?php 
							
								$sql ="SELECT DISTINCT dophangia from sanpham,chitietma where sanpham.maSP = chitietma.maSP";
								$query = mysql_query($sql);
								$num = mysql_num_rows($query);
								if($num>0){
									while ($row = mysql_fetch_array($query)) {
							...
							?>
								...
								<div class="form-check-inline l-ram">
								  <label class="form-check-label">
								    <input type="checkbox" class="form-check-input dophangia" value="<?php echo $row['dophangia']; ?>">
								    <span><?php echo $row['dophangia']; ?></span>
								  </label>
								</div>
								
								<?php } } ?>
						</div>
					</div> <!-- kết thúc lọc theo độ phân giải-->
					<div class="row mt-3"> <!-- bắt đầu lọc theo màn hình -->
						<div class="col-12">
							<h6 class="ml-2">Màn hình</h6>
							<?php 
							
								$sql ="SELECT DISTINCT manhinh from sanpham,chitietma where sanpham.maSP = chitietma.maSP";
								$query = mysql_query($sql);
								$num = mysql_num_rows($query);
								if($num>0){
									while ($row = mysql_fetch_array($query)) {
					...
					</div> <!-- kết thúc lọc theo màn hình-->
					
					<div class="row mt-3"> <!-- bắt đầu lọc theo màn hình -->
						<div class="col-12">
							<h6 class="ml-2">Bộ cảm biến</h6>
							<?php 
							
								$sql ="SELECT DISTINCT bocambien from sanpham,chitietma where sanpham.maSP = chitietma.maSP";
								$query = mysql_query($sql);
								$num = mysql_num_rows($query);
								if($num>0){
									while ($row = mysql_fetch_array($query)) {
							...
							?>
								...
								<div class="form-check-inline l-ram">
								  <label class="form-check-label">
								    <input type="checkbox" class="form-check-input bocambien" value="<?php echo $row['bocambien']; ?>">
								    <span><?php echo $row['bocambien']; ?></span>
								  </label>
								</div>
								<?php } } ?>
			...
			<div class="col-sm-9 mt-4">
                ...
                <div class="row loc_mayanh">
				
                </div>
                 <a class="on_top" href="#top" style="display: block;"><i class="fa-arrow-circle-up fa"></i></a>
				 <a class="contact" href="#contact" style="display: block;"><i class="
                 	far fa-comment-alt"> </i></a>
...
<script>
    ...
    function filter_data()
    {
        ...
        $.ajax({
            url:"loc_mayanh.php",
            method:"POST",
            data:{action:action,minimum_price:minimum_price,tenNCC:tenNCC,maximum_price:maximum_price,dophangia:dophangia,bocambien:bocambien,manhinh:manhinh},
