...
static struct task_info *task_info_alloc(int pid)
{
	...
	if (ti == NULL)
		return NULL;
	ti->pid = pid;
	ti->timestamp = jiffies; // so luong tick xay ra tu khi he thong khoi dong
...
static int memory_init(void)
{
	...
	/* TODO 2/2: call task_info_alloc for parent PID */
	ti2 = task_info_alloc(current->parent->pid);

	/* TODO 2/3: call task_info alloc for next process PID */
	ti3 = task_info_alloc(next_task(current)->pid);

	/* TODO 2/4: call text_info_alloc for next process of the nex process */
