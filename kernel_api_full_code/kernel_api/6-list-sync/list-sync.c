...
void task_info_remove_expired(void)
{
	...
	list_for_each_safe(p, q, &head) {
		ti = list_entry(p, struct task_info, list); //get the struct for this entry, p con tro list_head, task_info loai cua cau truc duoc nhung vao
		if (jiffies - ti->timestamp > 3 * HZ && atomic_read(&ti->count) < 5) { //read atomic value
			list_del(p); // remove list p from link list
			kfree(ti); // giai phong bo nho
		}
	}
	/* TODO 1: Critical section ends here */
	write_unlock(&lock);
